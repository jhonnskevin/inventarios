...
class TransaccionController extends Controller
{
    ...
    public function create_salida()
    {
        ...
        // Logica de número de comprobante
        /*
        $nuevoNumeroComprobante = Configuracion::where('id', 1)->first();
        $actualizaNroComprobante = Configuracion::where('id', 2)->first();

        if ($actualizaNroComprobante->valor == 'Si')
        {
            $numeroComprobante = $nuevoNumeroComprobante->valor;

            $actualizaNroComprobante->valor = 'No';
            $actualizaNroComprobante->save();
        } else {
            $ultimaSalida = Transaccion::where('tipo', 'Venta')->latest()->first();
            $numeroComprobante = $ultimaSalida ? $ultimaSalida->numero_comprobante + 1 : 1;
        }
        */
        $numeroComprobante = 1;
